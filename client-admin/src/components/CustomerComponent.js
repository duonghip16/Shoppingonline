    ...
    const customers = this.state.customers.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trCustomerClick(item)}>
          ...
          <td>
            {item.active === 0 ?
              <span className="link font-bold" onClick={() => this.lnkEmailClick(item)}>EMAIL</span>
              :
              <span className="link font-bold" onClick={() => this.lnkDeactiveClick(item)}>DEACTIVE</span>}
          </td>
        </tr>
      );
    });
    ...
    return (
      <div>
        <div className="align-center">
          <h2 className="text-center font-lato font-bold text-2xl text-red">CUSTOMER LIST</h2>
